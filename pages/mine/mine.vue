<template>
	<view class="content">
		<view class="header" v-bind:class="{'status':isH5Plus}">
			...
			<view class="setting"><image src="../../static/img/shezhi.png"></image></view>
		</view>
		...
		<view class="list" v-for="(list,list_i) in severList" :key="list_i">
			<view class="li" v-for="(li,li_i) in list" @tap="toPage(list_i,li_i)" v-bind:class="{'noborder':li_i==list.length-1}"  hover-class="hover" :key="li.name" >
				<view class="icon"><image :src="'../../static/img/icon/'+li.icon"></image></view>
				<view class="text">{{li.name}}</view>
				<image class="to" src="../../static/img/to.png"></image>
			</view>
		</view>
		<button class="mine-btn" type="warn" size="default">登录领返现</button>
	</view>
</template>
<script>
	export default {
...
.mine-price{
	background-image: url('../../static/img/shezhi.png');
	height:100upx;
}
.list{
